...
public class ClaviusUtils {
	...
	private static void handleAnalysisFields(String line, Element doc, int count) {
		try {
			doc.getChildren()
					.get(8)
					.addContent(
							makeElement(line.split("\t")[0],
									line.split("\t")[1], line.split("\t")[2],
									count, doc.getChildren("field").get(1)
											.getText()));
			doc.getChildren()
					.get(9)
					.addContent(
							makeElement(
									...
									line.split("\t")[1]
											+ line.substring(line
													.lastIndexOf(9)),
									line.split("\t")[2], count, doc
											.getChildren("field").get(1)
											.getText()));
					...
					.addContent(
							makeElement(
									...
									line.split("\t")[0]
											...
											.toLowerCase()
											+ "*", count,
									doc.getChildren("field").get(1).getText()));
			doc.getChildren()
					.get(9)
					.addContent(
							makeElement(
	...
	private static Element makeElement(String token, String pos, String lemma,
			int count, String sentence) {
		return new Element("w")
				.setAttribute("prog", String.valueOf(count))
				.setAttribute(
						"form",
						token.substring(token.indexOf('@') + 1,
								token.indexOf('[')).toLowerCase())
				...
				.setAttribute("start",
						String.valueOf(handleOffset(token, sentence, 0)))
				.setAttribute("end",
						String.valueOf(handleOffset(token, sentence, 1)));
	}
	...
	public static int handleOffset(String ctsToken, String sentence, int soe) {
		int ret = 0;
		int count = 0;
		int times = Integer.parseInt(ctsToken.substring(
				ctsToken.indexOf('[') + 1, ctsToken.indexOf(']')));
		System.err.println(times);
		String literalToken = ctsToken.substring(ctsToken.indexOf('@') + 1,
				ctsToken.indexOf('['));
		String patternQuote = "";
		...
		// FIXME attenzione la regex deve avere i boudary del token, altrimenti
		// matcha alche le sottostringhe falsando gli offset
		Pattern pa = Pattern.compile(patternQuote);
		Matcher ma = pa.matcher(sentence);
		System.out.println(pa.pattern());
		/*
		 * se il token esiste devo prendere la sua giusta occorrenza, quindi
		 * scorro i founds finchè non raggiungo l'occorrenza corretta valutando
		 * la variabile times
		 */
		while (ma.find()) {
			count = count + 1;
	...
	private static void handleSentenceFields(String line, List<Element> fields) {
		/*
		 * be care the file in input has to start with the correct information
		 * with out any header
		 */

		fields.get(0).getAttribute("name").setValue("id");
		fields.get(0).setText(
				line.substring(line.indexOf("s_") + 2, line.indexOf(":: ")));
		...
		fields.get(2).setText(
				HandleConstants.getLetterRif()
						+ "-"
						+ line.substring(line.indexOf("s_"),
								line.indexOf(":: ")) + ".png"); // OK for
																// Clavius
		// fields.get(2).setText(HandleConstants.getLetterRif()+"-"+"xyz.png");
		// // OK for Virgilius
		...
		fields.get(4).setText(
				"CITE for " + line.substring(0, line.indexOf(":: ")));
		fields.get(5).getAttribute("name").setValue("info_sentence");
		fields.get(5).setText(
				"additional information for sentence "
						+ line.substring(0, line.indexOf(":: ")));
		fields.get(6).getAttribute("name").setValue("info_image");
		fields.get(6).setText(
	...
	private static List<Element> makeFields(int n) {
		...
		for (int i = 0; i < n; i++) {
			Element ele = new Element("field").setAttribute("name",
					String.valueOf(i));
			fields.add(ele);
		}
		return fields;
	...
	public static void makeSentenceXML(Document xmlSentences) {
		...
		for (Element doc : docs) {
			...
			try {
				xo.output(
						new Document().setRootElement(new Element("add")
								.setContent(tmp_doc)),
						new FileWriter(HandleConstants.getWorkDir() + "Letter"
								+ HandleConstants.getLetterRif() + "_an-"
								+ doc.getChildText("field") + ".xml"));
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static void makeIntegrationXMLforAnalysis(Document xmlDoc) {
		...
		try {
			teiDoc = TextUtils.fileToDocument(HandleConstants.getWorkDir()+HandleConstants.getTeifile());
		...
		for (Element doc : docs) {
			...
			String ctsUri = PopulateSentenceAnalysis(sentenceAnalysis, doc);
			sentecesAnalysis.add(sentenceAnalysis);
			// sentence offset is for handling start end in a letter scope
			// instead in a sentence scope
			String[] ctsParts = ctsUri.split("\\.");
			System.err.println("CTSURN PER GESTIONE TOKEN DA TEI **** \n"+ ctsParts[ctsParts.length-1]);

			Namespace ns = teiDoc.getRootElement().getNamespace();
			String pathToSentence = HandleConstants.getXpathForSentences()+"[@n='"+ctsParts[ctsParts.length-1]+"']";

			XPathExpression<Element> sentencesExpression = XPathFactory
					.instance().compile(pathToSentence, Filters.element(), null,
							Namespace.getNamespace("tei", ns.getURI()));
			List<Element> listOfSentences = sentencesExpression
					.evaluate(teiDoc);

			if (null != listOfSentences) {
				System.err.println("number of sentences: "
						+ listOfSentences.size());
				sentenceOffset = PopulateTokens(tokens, doc, listOfSentences.get(0),
						sentenceOffset);
			}
		}
		try {
			xo.output(new Document().setRootElement(new Element(
					"linguistical_analysis").setContent(sentecesAnalysis)),
					new FileWriter(HandleConstants.getWorkDir() + "Letter"
							+ HandleConstants.getLetterRif() + "_an.xml"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		try {
			xo.output(new Document().setRootElement(new Element("tokens")
					.setAttribute("uri", "").setContent(tokens)),
					new FileWriter(HandleConstants.getWorkDir() + "Letter"
							+ HandleConstants.getLetterRif() + "_tokens.xml"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private static Integer PopulateTokens(List<Element> toks, Element doc,
			Element teiSentence, Integer Offset) {
		...
		System.out
				.println("Offset: " + Offset + " localOffset: " + localOffset);
		return Offset;
	}
	...
	private static String handleAbbreviation(String tokenValue,
			Element teiSentence) {
		// TODO Auto-generated method stub
		Namespace ns = teiSentence.getDocument().getRootElement().getNamespace();
		String pathToChoice = "tei:choice";
		Element choice = null;
		String ret = "";

		XPathExpression<Element> choicesExpression = XPathFactory
				.instance().compile(pathToChoice, Filters.element(), null,
						Namespace.getNamespace("tei", ns.getURI()));
		...
		if( (!listOfChoice.isEmpty()) && (null!=listOfChoice) && (null!=listOfChoice.get(0).getChild("abbr", ns))){
			 ...
			 if(tokenValue.equals(choice.getChild("expan", ns).getText()))
			 	ret = choice.getChild("abbr", ns).getText();
		}
	...
	private static String PopulateSentenceAnalysis(Element newAnalysis,
			Element oldAnalysis) {
