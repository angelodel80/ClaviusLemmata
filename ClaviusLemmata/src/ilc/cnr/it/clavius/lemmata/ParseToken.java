...
public class ParseToken extends ParseText {
	...
	public static boolean elabora(String line){
		...
		String query = 
				...
				//"SELECT lemma_text, morph_code FROM hib_parses as f LEFT JOIN hib_lemmas as l on f.lemma_id = l.lemma_id WHERE f.form = \"{[?form?]}\""; 
		if( (null!= line) && (line.split("\t").length >= 2) ){
			String tokenCTS = line.split("\t")[0];
			String token = tokenCTS.substring(tokenCTS.indexOf('@')+1,tokenCTS.indexOf('['));
			...
			if(!("".equals(token))){
				//query = query+token+"\"";
				query = query.replaceFirst("\\{\\[\\?form\\?\\]\\}", token.replaceAll("\\\\", "\\\\\\\\\\\\\\\\")).replaceFirst("\\{\\[\\?pos\\?\\]\\}", pos);
				...
				try {
					...
					outString.append(String.format("\n%s\t%s\t%s\t%s\n", tokenCTS, morphoTag, lemma, morphos.toString()));
